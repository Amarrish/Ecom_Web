...
function MenuItems() {
  ...
  function handleNavigate(getCurrentMenuItem) {
    ...
    location.pathname.includes("listing") && currentFilter !== null? setSearchParams(
    new URLSearchParams(`?category=${getCurrentMenuItem.id}`)): navigate(getCurrentMenuItem.path);
  }

  return (
    <nav className="flex text-xs flex-col mb-3 lg:mb-0 lg:items-center gap-6 lg:flex-row">
      {shoppingViewHeaderMenuItems.map((menuItem) => (
        <Label onClick={() => handleNavigate(menuItem)} className="text-sm font-medium cursor-pointer" 
        key={menuItem.id} >
       {menuItem.label}
        </Label>
      ))}
...
function HeaderRightContent() {
  ...
  useEffect(() => {
    if (user?.userId) dispatch(fetchCartItems(user.userId));
      ...
      <div className="flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" size="icon" className="lg:hidden"> {/* Menubar */}
              <Menu className="h-6 w-6 " />
              <span className="sr-only bg-amber-400">Toggle header menu</span>
            </Button>
          </SheetTrigger>
